...
const isValidAlarmType = (value: unknown): value is "display" | "audio" | "procedure" | "email" =>
...
class ReminderRepository {
  private mapReminder(reminder: ReminderJSON): Reminder {
    ...
    if (reminder.recurrenceRules && reminder.recurrenceRules.length > 0) {
      normalizedReminder.recurrenceRules = reminder.recurrenceRules.map(mapRecurrenceRule)
    }
        ...
        .map((alarm) => ({
          ...
          locationTrigger: alarm.locationTrigger
            ? {
                ...
                proximity: alarm.locationTrigger.proximity === "leave" ? "leave" : "enter",
              }
            : undefined,
        }))
    }
  ...
  async findReminderById(id: string): Promise<Reminder> {
    const reminderJSON = await executeCli<ReminderJSON>(["--action", "read-by-id", "--id", id])
  ...
  async updateReminderList(currentName: string, newName?: string, color?: string): Promise<ReminderList> {
