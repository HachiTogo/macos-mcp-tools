...
describe("subtaskUtils", () => {
  ...
  describe("parseSubtasks", () => {
    ...
    it("should return empty array for notes without subtask section", () => {
      ...
      expect(parseSubtasks("Notes with\nmultiple lines\nbut no subtasks")).toEqual([])
    })
...
---END SUBTASKS---`
  ...
  describe("serializeSubtasks", () => {
    ...
    it("should serialize a single incomplete subtask", () => {
      const subtasks: Subtask[] = [{ id: "abc12345", title: "Task one", isCompleted: false }]
    ...
    it("should serialize a single completed subtask", () => {
      const subtasks: Subtask[] = [{ id: "def67890", title: "Task done", isCompleted: true }]
      const result = serializeSubtasks(subtasks)
  ...
  describe("combineSubtasksAndNotes", () => {
    ...
    it("should return only subtasks if no notes", () => {
      const subtasks: Subtask[] = [{ id: "abc12345", title: "Task", isCompleted: false }]
    ...
    it("should replace existing subtask section", () => {
      const existingNotes = "My notes\n---SUBTASKS---\n[ ] {old1234} Old task\n---END SUBTASKS---"
      const newSubtasks: Subtask[] = [{ id: "new56789", title: "New task", isCompleted: false }]
  ...
  describe("updateSubtask", () => {
    ...
    it("should update subtask title", () => {
      const result = updateSubtask("abc12345", { title: "New title" }, existingNotes)
    ...
    it("should update subtask completion status", () => {
      const result = updateSubtask("abc12345", { isCompleted: true }, existingNotes)

      expect(result).toContain("[x] {abc12345}")
    })

    it("should update both title and completion", () => {
      const result = updateSubtask("abc12345", { title: "Updated", isCompleted: true }, existingNotes)

      expect(result).toContain("[x] {abc12345} Updated")
    })

    it("should throw error if subtask not found", () => {
      expect(() => updateSubtask("nonexistent", { title: "New" }, existingNotes)).toThrow(
  ...
  describe("removeSubtask", () => {
    ...
    it("should throw error if subtask not found", () => {
      expect(() => removeSubtask("nonexistent", existingNotes)).toThrow("Subtask with ID 'nonexistent' not found.")
    })

    it("should remove entire subtask section if last subtask removed", () => {
      const singleNotes = "Notes\n---SUBTASKS---\n[ ] {abc12345} Only task\n---END SUBTASKS---"
  ...
  describe("toggleSubtask", () => {
    ...
    it("should throw error if subtask not found", () => {
      expect(() => toggleSubtask("nonexistent", existingNotes)).toThrow("Subtask with ID 'nonexistent' not found.")
    })
  })
  ...
  describe("reorderSubtasks", () => {
    ...
    it("should reorder subtasks according to provided order", () => {
      const result = reorderSubtasks(["ccc33333", "aaa11111", "bbb22222"], existingNotes)
    ...
    it("should throw error if ID in order does not exist", () => {
      expect(() => reorderSubtasks(["nonexistent", "aaa11111", "bbb22222"], existingNotes)).toThrow(
        "Subtask with ID 'nonexistent' not found.",
      )
    })

    it("should throw error if subtask ID is missing from order", () => {
      expect(() => reorderSubtasks(["aaa11111", "bbb22222"], existingNotes)).toThrow(
        "Reorder array is missing subtask ID 'ccc33333'",
      )
    })

    it("should handle single subtask", () => {
      const singleNotes = "---SUBTASKS---\n[ ] {aaa11111} Only\n---END SUBTASKS---"
