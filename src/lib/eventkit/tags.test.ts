...
describe("tagUtils", () => {
  describe("extractTags", () => {
    it("extracts tags in [#tag] bracket format", () => {
      expect(extractTags("[#work] [#urgent] Some notes")).toEqual(["work", "urgent"])
    })

    it("extracts bare #tag format (Apple Reminders native)", () => {
      expect(extractTags("#social #work Some notes")).toEqual(["social", "work"])
    })
    ...
    it("extracts mixed bracket and bare formats without duplicates", () => {
      expect(extractTags("[#work] #social #work Some notes")).toEqual(["work", "social"])
    })

    it("handles bare tags with underscores and hyphens", () => {
      expect(extractTags("#my-tag #my_tag notes")).toEqual(["my-tag", "my_tag"])
    })
  ...
  describe("stripTags", () => {
    ...
    it("preserves non-tag hash references like #42", () => {
      expect(stripTags("See issue #42 for details")).toBe("See issue #42 for details")
    })
