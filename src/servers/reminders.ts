...
function rebuildNotesForUpdate(
  ...
  if (tags) {
    const baseNote = newNote !== undefined ? stripSubtasks(stripTags(newNote)) : stripTags(notesWithoutSubtasks)
...
const formatAlarm = (alarm: Alarm): string => {
  ...
  if (alarm.relativeOffset !== undefined) return `${alarm.relativeOffset}s from due/start${typeStr}`
  if (alarm.locationTrigger) return `on ${formatLocationTrigger(alarm.locationTrigger)}${typeStr}`
  return "unknown"
}
...
const buildReminderIcons = (reminder: {
  ...
  const icons: string[] = []
  if (reminder.recurrenceRules && reminder.recurrenceRules.length > 0) icons.push("\u{1F504}")
...
const formatReminderMarkdown = (reminder: {
  ...
  const recurrenceRules = reminder.recurrenceRules ?? (reminder.recurrence ? [reminder.recurrence] : undefined)
  ...
  if (reminder.completionDate) lines.push(`  - Completed: ${reminder.completionDate}`)
  if (reminder.url) lines.push(`  - URL: ${reminder.url}`)
  if (reminder.externalId) lines.push(`  - External ID: ${reminder.externalId}`)
  if (reminder.creationDate) lines.push(`  - Created: ${reminder.creationDate}`)
  if (reminder.lastModifiedDate) lines.push(`  - Modified: ${reminder.lastModifiedDate}`)
  return lines
}

// ── List formatter ────────────────────────────────────────────────────

const formatReminderList = (list: { title: string; id: string; color?: string }): string[] => {
  ...
  "reminders_tasks",
  {
    ...
    inputSchema: {
      ...
      startDate: z
        .string()
        .optional()
        .describe("Start date. RECOMMENDED format: 'YYYY-MM-DD HH:mm:ss' (local time). Also supports ISO 8601"),
      dueDate: z
        .string()
        .optional()
        .describe("Due date. RECOMMENDED format: 'YYYY-MM-DD HH:mm:ss' (local time). Also supports ISO 8601"),
      ...
      location: z
        .string()
        .optional()
        .describe("Location text for the reminder (not the same as a location-based trigger)"),
      ...
      priority: z.number().int().optional().describe("Priority level: 0=none, 1=high, 5=medium, 9=low"),
      alarms: z
        .array(
          z.object({
      ...
      filterTags: z.array(z.string()).optional().describe("Filter reminders by tags (must have ALL specified tags)"),
      tags: z
        .array(z.string())
        .optional()
      ...
      removeTags: z.array(z.string()).optional().describe("Tags to remove from the reminder (for update)"),
      subtasks: z
        .array(z.string())
        .optional()
      ...
      clearRecurrence: z
        .boolean()
        .optional()
        .describe("Set to true to remove recurrence from an existing reminder (for update)"),
      locationTrigger: z
        .object({
      ...
      clearLocationTrigger: z
        .boolean()
        .optional()
        .describe("Set to true to remove location trigger from an existing reminder (for update)"),
    },
  },
  async (args) => {
    switch (args.action) {
      case "read":
        return handleAsyncOperation(async () => {
          ...
          if (validatedArgs.id) {
            ...
            const markdownLines: string[] = ["### Reminder", "", ...formatReminderMarkdown(reminder)]
      ...
      case "create":
        return handleAsyncOperation(async () => {
          ...
          const reminder = await reminderRepository.createReminder({
            ...
            recurrenceRules:
              validatedArgs.recurrenceRules ?? (validatedArgs.recurrence ? [validatedArgs.recurrence] : undefined),
  ...
  "reminders_lists",
  {
    ...
    inputSchema: {
      ...
      name: z
        .string()
        .optional()
        .describe("The current name of the list (for update, delete) or the name of the new list (for create)"),
  ...
  "reminders_subtasks",
  {
    ...
    inputSchema: {
      ...
      subtaskId: z
        .string()
        .optional()
        .describe("The unique identifier of the subtask (REQUIRED for update, delete, toggle)"),
