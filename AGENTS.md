...
## Commands
- Runtime is Bun on macOS only. Local entrypoint: `bun run src/cli.ts <mail|contacts|notes|tasks|memory|messages>`.
- Full tests: `bun test`
...
- Main server files are `src/servers/mail.ts`, `contacts.ts`, `notes.ts`, `tasks.ts`, `memory.ts`, and `messages.ts`.
...
- `messages.ts` reads from `~/Library/Messages/chat.db` (readonly SQLite) and uses JXA/`osascript` for sending messages.
...
- There is no automated coverage for live JXA behavior (`contacts`, `notes`, `messages`, mail mutations/attachments) or end-to-end `tasks`/`memory` database flows, so those changes need manual verification.
